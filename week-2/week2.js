...
const typeWater = pokemons.filter((pokemon)=>{
    return pokemon.type == "Water"
})

// Kaç tane Water türünde pokemon olduğunu bulmak için:
console.log(typeWater.length);

// Water türündeki pokemon'ların isimlerini bulmak için:
console.log(typeWater.forEach((i)=>{
    console.log(i.name)
}))
...
//a
const allWaterPokemonsHighExperience = typeWater.every((i)=>{
    i.experience >=60
})

console.log(allWaterPokemonsHighExperience)

//b

const typeFire = pokemons.filter((pokemon)=>{
    return pokemon.type == "Fire"
})

const someFirePokemonHighExperience = typeFire.some((i)=>{
    return i.experience >= 70
})
...
const typeElectric = pokemons.filter((pokemon)=>{
    return pokemon.type == "Electric"
})

const totalElectricExperience = typeElectric.reduce((total,i)=>{
        return total + i.experience
},0)
...
const typeExperienceTotal = pokemons.reduce((total,pokemon)=>{
    return total + pokemon.experience
},0)

const typeExperienceAverage = typeExperienceTotal / pokemons.length;

console.log(typeExperienceAverage)